<template>
    <div class="tab-pane" id="computers" aria-labelledby="computers-tab" role="tabpanel">
        <div class="card">
            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                <div class="d-flex justify-content-between align-items-center mx-0 row">
                    ...
                    <div class="col-sm-12 col-md-6 mb-1">
                        <div class="row d-flex" style="float: right;">
                            <div class="col-md-8">
                                <button style="margin-top: 7px;" class="btn btn-primary" data-toggle="modal" data-target="#add-policy-computer">Add Computer</button>
                            </div>
                            <div class="col-md-4">
                                <div class="ropdown dropdown-user" style="float: right;">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right" id="showMenu" aria-labelledby="dropdown-user">
                                        ...
                                        <a class="dropdown-item d-flex" href="javascript:void(0);">
                                            <div class="custom-control custom-checkbox"> 
                                                <input class="custom-control-input" v-model="computer_name" type="checkbox" id="computer_name">
                                                <label class="custom-control-label" for="computer_name">Computer</label>
                                            </div>
                                        </a>
                                        <a class="dropdown-item d-flex" href="javascript:void(0);">
                                            <div class="custom-control custom-checkbox"> 
                ...
                <table v-else class="datatables-basic table dataTable no-footer dtr-column" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                    <thead>
                        <tr role="row">
                            ...
                            <th class="text-center" v-if="computer_name">Computer</th>
                            <th class="text-center" v-if="enforced">Enforced</th>
                            <th class="text-center">Action</th>
                        </tr>
                    </thead>
                    <tbody v-if="show">
                        <tr v-for="(value,index) in policy_computers" v-bind:key="index">
                            <td class="text-center" v-if="priority"> {{ value.priority_policy }}</td>
                            <td class="text-center" v-if="computer_name">{{ value.computername.common_name }}</td>
                            <td class="text-center" v-if="enforced">{{ value.enforced_flag }}</td>
                            <td class="text-center">
                                <button type="button" @click="sendInfo(value.ts_id, value.policy_id)" data-toggle="modal" data-target="#delete-computer" class="btn">
                                    <i style="margin-top: 1px; color: red;" class="fa fa-trash"></i>
                                </button>
                                <!-- Delete Computer Modal -->
                                <div class="modal custom-modal fade" id="delete-computer" role="dialog">
                ...
                <div class="text-center" style="margin-top: 15px;" v-if="!isLoading && !show">
                    <h4>Oops! No Policy Computers Found</h4>
                </div>
                <pagination :pageData="policy_computers"></pagination>
            </div>
        </div>
        <add-computer :policy="policy"></add-computer>
    </div>
</template>
<script>
...
export default {
    ...
    components: { AddComputer, Pagination },
    data() {
        return {
            policy_computers: [],
    ...
    created() {
        var _this = this;
        this.get_policy_computers();
        EventBus.$on("policy-computers-added", function() {
            _this.get_policy_computers();
        });
    },
    ...
    methods: {
        ...
        get_policy_computers(page = 1) {
            ...
            axios
                ...
                .then(response => {
                    ...
                    this.policy_computers = arrayAllUsers;
        ...
        pageClicked(pageNo) {
            var vm = this;
            vm.get_policy_computers(pageNo);
        },
    ...
    computed: {
        show() {
            return this.policy_computers ? (this.policy_computers.length >= 1 ? true: false) : null
        },
