<template>
    <div class="tab-pane" id="ou" aria-labelledby="ou-tab" role="tabpanel">
        <div class="card">
            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                <div class="d-flex justify-content-between align-items-center mx-0 row">
                    ...
                    <div class="col-sm-12 col-md-6 mb-1">
                        <div class="row d-flex" style="float: right;">
                            <div class="col-md-8">
                                <button style="margin-top: 7px;" class="btn btn-primary" data-toggle="modal" data-target="#add-policy-ou">Add OU</button>
                            </div>
                            <div class="col-md-4">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right" id="showMenu" aria-labelledby="dropdown-user">
                                        ...
                                        <a class="dropdown-item d-flex" href="javascript:void(0);">
                                            <div class="custom-control custom-checkbox"> 
                                                ...
                                                <label class="custom-control-label" for="ou_name">OU</label>
                                            </div>
                                        </a>
                                        <a class="dropdown-item d-flex" href="javascript:void(0);">
                                            <div class="custom-control custom-checkbox"> 
                ...
                <table v-else class="datatables-basic table dataTable no-footer dtr-column" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                    <thead>
                        <tr role="row">
                            ...
                            <th class="text-center" v-if="ou_name">OU</th>
                            <th class="text-center" v-if="enforced">Enforced</th>
                            <th class="text-center">Action</th>
                        </tr>
                    </thead>
                    <tbody v-if="show">
                        <tr v-for="(value,index) in policy_ous" v-bind:key="index">
                            <td class="text-center" v-if="priority"> {{ value.priority_policy }}</td>
                            <td class="text-center" v-if="ou_name">{{ value.ousname.common_name }}</td>
                            <td class="text-center" v-if="enforced">{{ value.enforced_flag }}</td>
                            <td class="text-center">
                                <button type="button" @click="sendInfo(value.ts_id, value.policy_id)" data-toggle="modal" data-target="#delete-ou" class="btn">
                                    <i style="margin-top: 1px; color: red;" class="fa fa-trash"></i>
                                </button>
                                <!-- Delete OU Modal -->
                                <div class="modal custom-modal fade" id="delete-ou" role="dialog">
                ...
                <div class="text-center" style="margin-top: 15px;" v-if="!isLoading && !show">
                    <h4>Oops! No Policy OUs Found</h4>
                </div>
                <pagination :pageData="policy_ous"></pagination>
            </div>
        </div>
        <add-ou :policy="policy"></add-ou>
    </div>
</template>
<script>
import Pagination  from '../../../../pagination/pagination.vue'
import { EventBus } from "../../../../../vue-asset";
import AddOu from './AddOU.vue';

export default {

    props: ['policy'],

    components: { AddOu, Pagination },
    data() {
        return {
            policy_ous: [],
    ...
    created() {
        var _this = this;
        this.get_policy_ous();
        EventBus.$on("policy-ous-added", function() {
            _this.get_policy_ous();
        });
    },
    ...
    computed: {
        show() {
            return this.policy_ous ? (this.policy_ous.length >= 1 ? true: false) : null
        },
